# email_sender.py — Render the daily H&E News Clipping as HTML and email it.
#
...
def _resolve_report_links(report: Dict) -> None:
    ...
    news.google.com redirects are decoded to the real publisher URL via the
    gnewsdecoder library; direct-source URLs pass through; anything that
    can't be resolved falls back to a Google search URL with the headline.
    """
    ...
    print(f"  Link resolver: {counters['resolved']} decoded, "
          ...
          f"(of {len(all_items)} items)")

# 2026-05-22: switched from iCloud to Gmail. Requires Gmail App Password
# (not the account password) in EMAIL_APP_PASSWORD.
...
def render_markdown(report: Dict, events: List[Dict],
    ...
    out: List[str] = [
        "---",
        f"date: {date_str}",
        f"type: he-news-clipping",
        f"generated_by: H&E News Scraper Claude",
        f"timestamp: {now.isoformat()}",
        "---",
        "",
        f"# H&E News Clipping — {now.strftime('%d %b %Y')} ({now.strftime('%Hh%M')})",
        "",
        f"*{long_date}*",
        "",
        "---",
...
def save_to_vault(report: Dict, events: List[Dict],
    ...
    Default destination: ~\\Documentos\\Obsidian Vault\\raw\\clippings_he\\YYYY-MM-DD-HHMM.md
    (Separate folder from TMT clippings — no shared files.)
    """
    ...
    try:
        ...
        # Include run-time in filename so the 3 daily runs don't overwrite each other
        stamp    = datetime.now(LOCAL_TZ).strftime("%Y-%m-%d_%Hh%M")
        path     = os.path.join(vault_clippings_dir, f"{stamp}.md")
...
def render_html(report: Dict, events: List[Dict],
    ...
    parts.append(f'<h2 style="{_STYLE_TITLE}">H&amp;E News Clipping — {date_str}</h2>')
    ...
    parts.append(
        ...
        f'{now.strftime("%Y-%m-%d %H:%M %Z")} · H&amp;E News Scraper</p>'
    )
    parts.append("</div>")
...
def send(report: Dict, events: List[Dict],
    ...
    # Include run-time stamp (06h / 16h30 / 18h) so 3 daily emails don't collide
    run_time = datetime.now(LOCAL_TZ).strftime("%Hh%M")
    subject  = f"H&E News Clipping — {date_str} ({run_time})"
