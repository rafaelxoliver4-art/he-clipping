...
# Aliases for tickers Rafael writes with slight variations — keeps the
# scoreboard from forking (e.g. YDUQS and YDUQ are the same company).
_TICKER_ALIASES = {
    "YDUQS":   "YDUQ",
    "YDUQS3":  "YDUQ",
    "YDUQ3":   "YDUQ",
    ...
    "BLAU3":   "BLAU",
    "SAUD3":   "SAUD",
    "TOTS3":   "TOTVS",   # safety — in case TMT TOTS3 leaks in
}


def _normalize_name(name: str) -> str:
    """
    Canonicalize a topic name so variations collapse to one entry.
    'FISTEL' and 'Fistel' → 'FISTEL'. 'YDUQS' and 'YDUQ' both → 'YDUQ'.
    """
...
def _get_covered_names() -> set:
    """
    Build the canonical set of covered ticker symbols from config.SECTORS.
    Used by record_clipping_hits to bootstrap H&E topics from clipping activity.
    Returns uppercase set so case-insensitive matching works.
    """
    try:
        import config
        out: set = set()
        for sector_data in config.SECTORS.values():
            for ticker in sector_data.get("covered", []):
                if ticker:
                    out.add(ticker.strip().upper())
        return out
    except Exception:
        return set()
...
def record_clipping_hits(report: Dict) -> int:
    ...
    Bootstraps topics from covered-name tickers found in the clipping (so the
    H&E system can build its scoreboard from day 1 even before Rafael writes
    Observer notes or the vault discusses H&E themes).
    """
    state = _load_state()
    covered_set = _get_covered_names()

    # ── Bootstrap: auto-create topics for covered names found in today's clipping
    # This lets the system build state from its own daily output even when the
    # vault and Observer file are still empty of H&E content.
    bootstrapped = 0
    if covered_set:
        for sector_items in report.values():
            ...
            for item in sector_items:
                ticker_tag = (item.get("ticker") or "").strip()
                if not ticker_tag or ticker_tag.lower() == "sector":
                    continue
                # Split multi-ticker tags like "HAPV/RDOR" → ["HAPV", "RDOR"]
                for t in ticker_tag.split("/"):
                    t = _normalize_name(t.strip())
                    if t and t in covered_set and t not in state["topics"]:
                        desc = (item.get("headline") or "")[:200]
                        state["topics"][t] = _new_topic_record(t, desc)
                        bootstrapped += 1

    if not state["topics"]:
        # No bootstrap and no existing topics — nothing to reinforce
        return 0
