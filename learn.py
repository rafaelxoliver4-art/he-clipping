...
# ── System prompt ──────────────────────────────────────────────────────────────
LEARN_SYSTEM = """You are a senior LatAm Healthcare & Education (H&E) equity
research analyst helping maintain an automated news curator for UBS LatAm
H&E equity research.
...
COVERED UNIVERSE for this pipeline (12 names):
  Hospitals/Services: RDOR (Rede D'Or), ONCO (Oncoclínicas)
  Health Plans:       HAPV (Hapvida), ODPV (Odontoprev), SAUD (Bradsaúde)
  Diagnostics:        FLRY (Fleury)
  Pharma:             BLAU (Blau Farmacêutica)
  Higher Education:   YDUQ (YDUQS), COGN (Cogna), ANIM (Ânima), AFYA, LAUR

CRITICAL FILTERING RULE: Rafael covers BOTH TMT AND H&E. The Obsidian vault
and Claude Code session histories you receive will be heavily TMT-focused
(AMX, TIM, TOTVS, GLOB, CINT, etc.). For THIS pipeline you must IGNORE all
TMT-specific signal and only act on H&E-relevant material. If the vault is
all-TMT in a given window, return ANALYST_CONTEXT unchanged.

You receive eight inputs:
  A. Current ANALYST_CONTEXT (the H&E curator's knowledge today)
  B. Pipeline run log — H&E tickers/topics that were curated most, which notes were written
  C. News Writer session excerpts — Rafael's real editorial triage decisions
     from the Obsidian vault (FILTER FOR H&E NAMES ONLY)
  D. Claude Code session excerpts — H&E-relevant content only
  E. Real clipping examples — past H&E News Clippings (starts empty; populated as we go)
  F. Corpus analysis — H&E ticker frequencies, sector mix, source quality
  G. Claude Data reference docs — Rafael's source-of-truth files (treat
     anything TMT-focused as out of scope for H&E)
  H. Observer published-notes ANALYSIS — Rafael's H&E Observer notes
     (will be empty until he starts adding H&E notes). This is the STRONGEST
     signal when populated.

Your task: find concrete H&E refinements to ANALYST_CONTEXT clearly supported
by C, D, E, F, G, H — filtered for H&E coverage names only.

Look for (H&E-specific):
- H&E tickers or topics Rafael engages with that aren't in the context
- ANS / MEC / ANVISA rulings he treats as material
- Read-across logic: GLP-1 cost pressure on HAPV, EAD rule changes on YDUQ/COGN/ANIM
- Hospital-network expansion patterns (RDOR), payer M&A (HAPV), medicine-vagas
  decisions (YDUQ/ANIM/AFYA)
- New "active running stories" specific to H&E

Rules:
- Preserve ALL existing content — only add or refine, never delete
- Only add things clearly evidenced in the H&E-relevant sessions, not TMT carryover
- 2–6 well-targeted H&E additions beat 20 vague ones
- Don't change structure or section headers
- If C/D/E/F/H show no H&E signal, return ANALYST_CONTEXT unchanged

OUTPUT: Return ONLY the updated ANALYST_CONTEXT as a plain string — everything
that would appear between the triple quotes. No Python syntax, no fences, no preamble.
Start directly with "## UBS LatAm H&E..." (the first line of the current content)."""
...
# For H&E we ONLY load editorial-framework pages that apply to any sector
# (format spec, writing process, general note framework). TMT-specific pages
# (latam-tmt-*, news-writing-telecom-methodology, ai-disruption-*, anatel-*,
# latam-it-services-*, etc.) are intentionally EXCLUDED to prevent TMT noise
# from leaking into H&E context refinement.
_WIKI_PRIORITY_PAGES   = [
    "NEWS_WRITER_CONTEXT.md",        # master editorial framework (universal)
    "tmt-report-format.md",          # format spec (universal — Observer style)
    "news-writing-process.md",       # editorial workflow (universal)
]
# H&E-specific wiki pages — added here as Rafael creates them in the vault
_WIKI_HE_SPECIFIC_PAGES = [
    "he-coverage-universe.md",       # placeholder; create later
    "latam-he-coverage-universe.md", # alt naming
    "news-writing-health-methodology.md",
    "news-writing-education-methodology.md",
    "ans-cycle-reference.md",
    "mec-cycle-reference.md",
]
_MAX_WIKI_TOTAL_CHARS  = 12000   # tighter budget — fewer pages loaded
_MAX_WIKI_PAGE_CHARS   = 3000    # per-page cap (more depth per page)
...
def load_news_writer_context() -> str:
    ...
    # 1. Universal priority pages first
    seen: set = set()
    ordered = list(_WIKI_PRIORITY_PAGES)
    # 2. H&E-specific pages (if Rafael creates them)
    ordered.extend(_WIKI_HE_SPECIFIC_PAGES)
    # 3. Opportunistically include any other H&E-related .md page in wiki/
    # SKIP all TMT-specific pages to prevent TMT noise leaking into H&E refinement
    _TMT_SKIP_TOKENS = (
        "tmt-", "telecom", "telco", "anatel", "spectrum", "fistel",
        "amx-", "tim-", "vivo-", "totvs-", "globant", "ci&t",
        "ai-disruption", "ai-software", "it-services",
        "latam-tmt", "tmt_", "tmt.md",
    )
    try:
        for fname in sorted(os.listdir(_WIKI_DIR)):
            if not fname.endswith(".md") or fname in ordered:
                continue
            lower = fname.lower()
            if any(tok in lower for tok in _TMT_SKIP_TOKENS):
                continue
...
_TOPIC_EXTRACTOR_SYSTEM = """You are extracting time-sensitive topics for the UBS
LatAm Healthcare & Education (H&E) daily news scraper.

H&E COVERAGE UNIVERSE (the ONLY tickers that matter for this extractor):
  Hospitals:        RDOR (Rede D'Or), ONCO (Oncoclínicas)
  Health Plans:     HAPV (Hapvida), ODPV (Odontoprev), SAUD (Bradsaúde)
  Diagnostics:      FLRY (Fleury)
  Pharma:           BLAU (Blau Farmacêutica)
  Higher Education: YDUQ (YDUQS), COGN (Cogna), ANIM (Ânima), AFYA, LAUR

⚠ CRITICAL FILTER: Rafael also covers TMT (telecom/media/tech). His Obsidian
vault conversations are PREDOMINANTLY TMT-focused (AMX, TIM, TOTVS, GLOB,
CINT, V.tal, Anatel, FISTEL, spectrum auctions, AI/software disruption, etc.).

For THIS extractor you must IGNORE all TMT content and only surface topics
affecting the H&E universe above. If the conversation excerpts are 100% TMT,
return an empty result.

Look for H&E topics Rafael is actively discussing — regulatory proceedings,
pending deals, hot debates, sector themes, recurring concerns — that are NOT
already explicit in the curator's current ANALYST_CONTEXT.

Examples of GOOD H&E topics:
  - ANS reajuste cycle for individual plans (affects HAPV/SAUD MLR trajectory)
  - MEC EAD portaria tightening (affects YDUQ/COGN/ANIM captação)
  - Medicine-vagas authorization batch (affects YDUQ/ANIM/AFYA)
  - GLP-1 rol-inclusion debate (affects HAPV cost pressure)
  - Hospital network M&A pipeline (RDOR / Dasa / Mater Dei)
  - PCDT high-cost-medicine ruling (affects BLAU SUS-channel)
  - Specific ongoing M&A deals waiting for ANS/CADE approval
  - Tax-reform clauses for healthcare or education services
  - Specific STJ/STF rulings affecting payer cost structure

Examples of BAD topics to REJECT (these are TMT, not H&E):
  - FISTEL, 700 MHz auction, spectrum auctions, V.tal stake sale
  - Millicom / Telefónica / América Móvil M&A
  - TOTVS-Linx, GLOB/CINT AI strategy, Anthropic/OpenAI/SAP pricing
  - Chile 4-to-3 telecom consolidation, Mexico CRT / Altán
  - Anything about TMT/telco/IT-services/software companies

OUTPUT FORMAT — return ONLY H&E topics, one per line, no preamble:
- **TOPIC NAME** — why it matters, which H&E ticker(s) affected, what to watch for.

Each topic ~20-40 words. Return [] (or nothing) if there are no H&E topics
in the conversations. Do NOT pad output with TMT topics."""
