...
MAX_PER_SOURCE    = 30   # was 10. Bumped 2026-05-22 — same rationale as TMT.
...
def _parse_rss(xml_bytes: bytes, source_name: str, sector: str,
    ...
    def _parse_pubdate(raw: str):
        """Return:
          - "YYYY-MM-DD HH:MM"  if date parsed and within window
          - "TOO_OLD"           if parsed but older than cutoff (drop)
          - ""                  if undated (KEEP with empty date marker)
        """
        if not raw:
            return ""
        try:
...
def _fetch_html(source: Dict) -> List[Dict]:
    ...
    # 2026-05-22 fix: extract dates from URLs AND from listing HTML context.
    import re as _re
    DATE_PATTERNS = [
    ...
    def _date_from_ctx(ctx: str):
        """Extract date from listing-HTML chunk around a link (post-anchor)."""
        for pat, builder in CTX_PATTERNS:
            ...
            if m:
                try:
                    ...
                    pass
        # Written-out month: "18 May 2026" / "May 18, 2026" / "18 de mayo de 2026"
        ...
        for m in _re.finditer(r'\b([A-Za-z]+)\s+(\d{1,2}),?\s+(\d{4})\b', ctx):
            ...
            if mo_num and 2020 <= int(y) <= 2030 and 1 <= int(d) <= 31:
                from datetime import date as _date
                return _date(int(y), mo_num, int(d))
        # Numeric DD/MM/YYYY (Brazilian / LatAm convention).
    ...
    for h in candidates:
        ...
        if pub_date is None and h.get("link"):
            link = h["link"]
            idx = html.find(link)
            # 2026-05-22 fix: handle sites with RELATIVE URLs (/noticias/foo).
            if idx < 0:
                try:
                    from urllib.parse import urlparse
                    p = urlparse(link)
                    ...
                    pass
            if idx >= 0:
                # Fixed 5000-char window starting at idx. First date in the
                # window is always this article's date (page renders top-down).
                # More robust than </a> or path-prefix boundaries.
        ...
        if pub_date is not None:
            ...
            # No date in URL or context — keep with empty marker; downstream
            # renders as "(date unknown)" and curator decides materiality.
            pub_str = ""
