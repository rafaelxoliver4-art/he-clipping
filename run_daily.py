# run_daily.py — H&E News Scraper: full end-to-end pipeline
# (Healthcare & Education — separate from TMT, no shared state)
#
# Usage:
...
# ── Sample data for --test (H&E) ──────────────────────────────────────────────
SAMPLE_ROWS = [
    {"title": "ANS publica RN com novo limite de reajuste para planos individuais",
     "source": "Valor", "link": "https://valor.globo.com/empresas/saude", "keyword": "ANS reajuste",
     "published_local": datetime.now(LOCAL_TZ).strftime("%Y-%m-%d"),
     "edition_lang": "pt-BR", "edition_country": "BR", "source_type": "gnews"},
    {"title": "Hapvida anuncia aquisição de operadora regional no Nordeste por R$1bn",
     "source": "Brazil Journal", "link": "https://braziljournal.com", "keyword": "Hapvida",
     "published_local": datetime.now(LOCAL_TZ).strftime("%Y-%m-%d"),
     "edition_lang": "pt-BR", "edition_country": "BR", "source_type": "gnews"},
    {"title": "Rede D'Or inaugura novo hospital em Brasília e amplia rede para 75 unidades",
     "source": "Pipeline Valor", "link": "https://pipelinevalor.globo.com", "keyword": "Rede D'Or",
     "published_local": datetime.now(LOCAL_TZ).strftime("%Y-%m-%d"),
     "edition_lang": "pt-BR", "edition_country": "BR", "source_type": "direct"},
    {"title": "MEC publica portaria endurecendo critérios para autorização de cursos EAD",
     "source": "JOTA", "link": "https://jota.info", "keyword": "MEC EAD",
     "published_local": datetime.now(LOCAL_TZ).strftime("%Y-%m-%d"),
     "edition_lang": "pt-BR", "edition_country": "BR", "source_type": "direct"},
    {"title": "Eli Lilly Q1 beats on Mounjaro sales; raises FY guidance",
     "source": "Reuters", "link": "", "keyword": "Eli Lilly",
     "published_local": datetime.now(LOCAL_TZ).strftime("%Y-%m-%d"),
     "edition_lang": "en", "edition_country": "US", "source_type": "gnews"},
    {"title": "UnitedHealth Q1 MLR ticks higher; cites elevated outpatient utilization",
     "source": "WSJ", "link": "", "keyword": "UnitedHealth",
     "published_local": datetime.now(LOCAL_TZ).strftime("%Y-%m-%d"),
     "edition_lang": "en", "edition_country": "US", "source_type": "gnews"},
    {"title": "STF decide a favor de operadora em ação sobre cobertura de medicamento de alto custo",
     "source": "JOTA", "link": "", "keyword": "judicialização",
     "published_local": datetime.now(LOCAL_TZ).strftime("%Y-%m-%d"),
     "edition_lang": "pt-BR", "edition_country": "BR", "source_type": "direct"},
    {"title": "YDUQS abre novo polo em Pernambuco e diz manter ritmo de captação",
     "source": "Folha Educação", "link": "", "keyword": "YDUQS",
     "published_local": datetime.now(LOCAL_TZ).strftime("%Y-%m-%d"),
     "edition_lang": "pt-BR", "edition_country": "BR", "source_type": "direct"},
    {"title": "Fleury fecha parceria com Hospital Sírio-Libanês em serviços de diagnóstico",
     "source": "Valor Saúde", "link": "", "keyword": "Fleury",
     "published_local": datetime.now(LOCAL_TZ).strftime("%Y-%m-%d"),
     "edition_lang": "pt-BR", "edition_country": "BR", "source_type": "direct"},
    {"title": "Hapvida Q1 EBITDA cresce 12% e supera estimativas do consenso",
     "source": "InfoMoney", "link": "", "keyword": "Hapvida",
     "published_local": datetime.now(LOCAL_TZ).strftime("%Y-%m-%d"),
     "edition_lang": "pt-BR", "edition_country": "BR", "source_type": "direct"},
]
...
def main():
    ...
    banner(f"H&E News Scraper  —  {start.strftime('%Y-%m-%d %H:%M %Z')}")
    ...
    try:
        ...
        # Skip cross-run dedup in modes that don't actually deliver the clipping
        # — TEST_MODE (sample data), NO_EMAIL (preview only), DRY_EMAIL (HTML
        # preview only). This stops preview runs from poisoning the next real
        # run's "seen" memory.
        is_preview = TEST_MODE or NO_EMAIL or DRY_EMAIL
        merged_rows, claude_input = merge_run(
            gnews_rows, direct_rows,
            apply_cross_run_dedup=not is_preview,
        )
        if is_preview:
            print(f"  (cross-run dedup SKIPPED — preview mode, dedup memory not updated)")
    ...
    try:
        ...
        total = rc["matched"] + rc["headline_only"] + rc.get("substring", 0) + rc["unmatched"]
        if total:
            print(f"  Link re-attach: {rc['matched']} exact, "
                  f"{rc['headline_only']} headline-only, "
                  f"{rc.get('substring',0)} substring, "
                  f"{rc['unmatched']} unmatched (of {total} items)")
        # Enforce 'max 5 EXT items' rule as a safety net.
    ...
    # Freshness verification + article-time enrichment (2026-05-26):
    # Fetch each curated item's article page to (a) correct the displayed
    # time to the precise value and (b) DROP items whose real pubdate is
    # older than the lookback window. Fixes the stale-leak bug where HTML
    # scrapers assigned "today" to weeks-old "recommended" sidebar links.
    try:
        ...
        if all_items:
            ...
            # news.google.com link and verify_freshness skips them (it can't
            # read a date off the Google redirect) — which let a 2-month-old
            # Estadão article leak in dated "today". Resolution is cached, so
            ...
            if stale:
                # Remove stale items from every sector
