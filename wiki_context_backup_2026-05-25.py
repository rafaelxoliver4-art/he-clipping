...
- ONCO (Oncoclínicas): Cancer-care network roll-up. Triggers: clinic
  ...
  thread — material when scope or eligibility is clarified.
...
- BLAU (Blau Farmacêutica): Hospital-channel specialty pharma (biológicos,
  ...
  flag:** corpus shows BLAU curated ~40x but Rafael's actual notes are
  concentrated in ONCO/HAPV/YDUQ/LAUR — meaning most BLAU items the curator
  flags are not material to the analyst. Default any BLAU/ANVISA item that
  is NOT (i) SUS-tender-related, (ii) biosimilar-segment-specific to a BLAU
  product, (iii) judicialização-related, or (iv) acting on BLAU itself —
  to Other News.
...
land or when ANS signals coverage intent.
...
12. **"ONCO credit / going-concern crisis"** — 2026 story line. Trigger items:
    ...
    counterparties). Default to full note on any concrete development.
...
- **Geography matters**: MG (Minas Gerais), SP, MS, and Litoral-region
  outbreak stories recur in the corpus — when state-level case counts
  spike, that is the named transmission. Generic "Brazil reports X cases"
  without operator or state-level angle = Other News or omit.
...
- **ANVISA (49x, 0 notes):** Tighten. Default ANVISA drug approvals belong in
...
- **BLAU (42x, 3 notes — promotion ~7%):** Tighten. Most BLAU items the
...
- **FLRY (29x, 1 note — promotion ~3%):** Tighten — see Diagnostics section
...
- **ANS / HAPV/SAUD (22x / 52x, 0 notes as the multi-tag combo):** Expand.
  ...
  + reajuste-cap RNs are the highest-conversion candidates.
- **ODPV (10x, 0 notes):** Hold the bar. Rafael's note set does not currently
...
- **MEC / YDUQ-COGN-ANIM (9x / 16x, 0 notes as the multi-tag combo):** Expand.
  Any concrete MEC EAD portaria with operator-named impact should be a full
  note, not a Sector tag. New Enade / CPC / IGC prints with EAD-vs-presencial
  gap data are also full-note candidates for the cohort.
...
- **GLP-1 (11x, 0 notes):** Expand for items with explicit BR transmission
  ...
  Nordisk pricing campaigns ahead of patent cliff). Generic US Lilly/Novo
  earnings recaps stay omitted.
