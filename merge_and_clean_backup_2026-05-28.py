...
def enforce_covered_inclusion(report: dict, raw_rows: list, max_add: int = 10) -> dict:
    """Server-side safety net (added 2026-05-25): force-include any DIRECT
    row whose title contains a covered name. Tagged with the proper ticker.
    ...
    for row in raw_rows:
        if counter["added"] >= max_add:
            break
        # Strict source check: only exact name / brand-exact / domain match
        if not _is_direct_strict(row, direct_info):
            continue
        ...
        if tk in added_keys:
            continue
        added_keys.add(tk)
        sec_name = "Forced inclusion (covered name in DIRECT source)"
