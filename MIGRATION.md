# Migrating the H&E Clipping to a new PC

This pipeline runs unattended on a Windows PC via Task Scheduler. Use this guide
to rebuild it on another machine. (TMT is a separate repo — repeat there too.)
...
- **Option A — OneDrive sync:** let OneDrive sync
  `...\OneDrive\Área de Trabalho\Python\News Scraper H&E Claude\` to the new PC.
- **Option B — git clone:**
  ```
  git clone https://github.com/rafaelxoliver4-art/he-clipping.git
  ```
...
Task definitions are saved in `scheduler\*.xml`. Import each (PowerShell, same user):
```powershell
Register-ScheduledTask -TaskName "H&E News Clipping 07-00 BRT" `
  -Xml (Get-Content "scheduler\H&E News Clipping 07-00 BRT.xml" -Raw)
Register-ScheduledTask -TaskName "H&E News Clipping 17-00 BRT" `
  -Xml (Get-Content "scheduler\H&E News Clipping 17-00 BRT.xml" -Raw)
```
...
- `<Command>` / `<Arguments>` — the `cd /d "<project folder>"`, the `python.exe`
  path, the `run_daily.py` path, and the `>> "<log path>"` redirect.
- `<WorkingDirectory>`.
...
(**06:40** morning, **17:00** afternoon, Mon–Fri).
...
```powershell
Disable-ScheduledTask -TaskName "H&E News Clipping 07-00 BRT"
Disable-ScheduledTask -TaskName "H&E News Clipping 17-00 BRT"
```
